...
curve_counter <- function(df, name_of_pop) {
  ...
  # Print to screen the first number of traits that is non-negative and the last number
  message("First number:")
  print(diff_intervals %>% select(difference, pleiotropy_score) %>% filter(difference > 0 & pleiotropy_score >= 2) %>% slice(which.min(pleiotropy_score)))
  message("Last number: ")
  print(diff_intervals %>% select(difference, pleiotropy_score) %>% filter(difference > 0 & pleiotropy_score >= 2) %>% slice(which.max(pleiotropy_score)))
  message("Mean:")
  print(diff_intervals %>% select(difference, pleiotropy_score) %>% filter(difference > 0 & pleiotropy_score >= 2) %>% summarise(mean = mean(pleiotropy_score), med = median(pleiotropy_score)))
  ...
  # aggregate metrics
  props <- cbind(name_of_pop, a, b, c, e) 
