...
val parser = ArgParser("inbred read counter")
val inFile by parser.option(ArgType.String, shortName = "i", description = "input file - sam aligned to parent 1").required()
...
val numNMProp by parser.option(ArgType.Double, shortName = "m", description = "maximum proportion of read length NM strict boundary").default(0.02)
val minMapQ by parser.option(ArgType.Int, shortName = "n", description = "minimum mapQ score").default(48)
parser.parse(args)


buildCountMatrixFile(inFile, transcriptList, outputFile, minAlignLengthProp, numNMProp, minMapQ)
